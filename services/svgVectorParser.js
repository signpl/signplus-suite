...
(function (root) {
  ...
  // 글자(문자) 하나에 해당하는 <g> 조상을 찾는다 — 바로 위(직계) <g>를 그 도형의 "글자 그룹"으로
  // 본다. 초성/중성/종성처럼 한 글자 안의 자소가 서로 안 겹치더라도, 같은 <g>를 공유하면 하나의
  // 글자로 정확히 묶인다(자소 여러 개가 모이면 그룹 bbox가 자소 하나보다 훨씬 커지는 것은 당연한
  // 결과라 크기 비율로는 "레이어 전체"와 "글자 하나"를 구분할 수 없어, 크기 비교 대신 바로 위
  // <g> 하나만 신뢰한다). 다만 그 <g>가 전체 도면의 70% 이상을 차지하면(=레이어/아트보드 전체를
  // 감싸는 그룹) 여러 글자를 한꺼번에 묶어버리는 것이므로 신호로 쓰지 않는다.
  function characterGroupOf(el, overallBBox) {
    ...
    if (!gBBox) return null;
    if (overallBBox && overallBBox.width > 0 && overallBBox.height > 0 && gBBox.width >= overallBBox.width * 0.7 && gBBox.height >= overallBBox.height * 0.7) {
      return null; // 레이어/아트보드 전체를 감싸는 그룹으로 보임 — 글자 판정 신호로 쓰지 않음
    }
    return node;
  }
