...
Always optimize for shipping speed.

Before executing any QA or verification step, determine whether it is required for release readiness.

Skip any QA that is redundant, low-value, or does not materially reduce release risk.

Do not repeat the same verification using different methods unless the previous verification failed.

Prefer functional verification over implementation verification.
...
- PDF/Export works (if changed)
- No crash or blocking error

Avoid unnecessary:

- Repeated screenshots
- Duplicate CDP/DOM checks
- Repeated process inspections
- CSS/HTML string comparisons
- Multiple ways of verifying the same result

Always choose the shortest path that provides sufficient confidence for release.
